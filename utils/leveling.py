...
class LevelRole(DiscordNamedObj):
    objType = DiscordNamedObjTypes.levelRole
...
class LevelRoleSet(DiscordNamedObjSet["LevelRole"]):
    ...
    def setType(self) -> DiscordNamedObjTypes:
        return DiscordNamedObjTypes.levelRole
            ...
            match confkey:
                case "level roles":
                    if confvalue and isinstance(confvalue, dict):
                        self.level_roles = {key: LevelRoleSet(
                                [LevelRole(level=subkey, id=subvalue) for subkey, subvalue in value.items()]
